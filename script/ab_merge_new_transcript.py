...
class Transcript:
    ...
    def __init__(self, transcript_id, chrom, strand, exons, cds_regions, utr5, utr3, source_file):
        ...
        self.chrom = chrom
        self.strand = strand
        self.exons = sorted(exons, key=lambda x: x[0])  # 按起始位置排序
...
class TranscriptProcessor:
                    ...
                    if line.startswith('>'):
                        ...
                        if match:
                            ...
                            if len(parts) >= 1:
                                ...
                                # 在描述中查找染色体信息
                                for part in parts:
                                    if 'chromosome:' in part:
                                        chrom = part.split('chromosome:')[-1]
                                        transcript_to_chrom[transcript_id] = chrom
                                        break
                                        # 如果没有找到chromosome信息，尝试其他模式
                                    elif 'chr' in part and len(part) <= 10:  # 简单的染色体标识
                                        chrom = part
                                        transcript_to_chrom[transcript_id] = chrom
                                        break
    ...
    def find_matching_file_pairs(self, gff3_dir, fasta_dir):
        ...
        for f in os.listdir(fasta_dir):
            if f.endswith('.fa') or f.endswith('.fasta') or f.endswith('.fna'):
                ...
                if base_name in gff3_files:
                    ...
                    # 尝试更宽松的匹配：移除可能的版本号等后缀
                    base_name_simple = base_name.split('.')[0]  # 取点号前的部分
                    for gff_base in gff3_files.keys():
                        if gff_base.startswith(base_name_simple) or base_name_simple.startswith(gff_base):
                            file_pairs.append({
                                'gff3': gff3_files[gff_base],
                                'fasta': os.path.join(fasta_dir, f),
        ...
        for pair in file_pairs:
            key = (pair['gff3'], pair['fasta'])
            if key not in seen:
                seen.add(key)
                unique_pairs.append(pair)
        
        print(f"找到 {len(unique_pairs)} 对匹配的文件")
        return unique_pairs
    ...
    def parse_gff3_file(self, gff3_file, transcript_to_chrom, source_name):
        ...
        try:
            ...
            for transcript in db.features_of_type('transcript'):
                ...
                # 获取染色体信息
                chrom = transcript_to_chrom.get(transcript_id)
                if not chrom:
                    # 如果直接匹配失败，尝试其他ID格式
                    for key in transcript_to_chrom.keys():
                        if transcript_id in key or key in transcript_id:
                ...
                # 获取CDS区域
                cds_regions = []
                for cds in db.children(transcript_id, featuretype='CDS'):
                    cds_regions.append((cds.start, cds.end))
                
                # 获取UTR区域
                utr5 = []
                utr3 = []
                for utr in db.children(transcript_id, featuretype=['five_prime_UTR', 'three_prime_UTR']):
    ...
    def process_file_pairs(self, file_pairs):
        ...
        for i, pair in enumerate(file_pairs):
            ...
            if not transcripts:
                print(f"  警告: 未从GFF3文件中找到有效转录本: {pair['gff3']}")
                continue
            ...
            for transcript in transcripts:
                ...
                if fingerprint not in self.fingerprint_dict:
                    ...
                    self.fingerprint_dict[fingerprint]['support_files'].append(pair['base_name'])
    ...
    def _generate_nonredundant_gff3(self, output_dir):
        ...
        with open(output_file, 'w') as f:
            ...
            for fingerprint, info in sorted(self.fingerprint_dict.items(), 
                ...
                f.write(f"ID={transcript.transcript_id};Support_Count={info['count']};Source_Files={','.join(info['support_files'][:5])}{'...' if len(info['support_files']) > 5 else ''}\n")
    ...
    def _generate_support_details(self, output_dir):
        ...
        with open(output_file, 'w') as f:
            f.write("Fingerprint\tRepresentative_Transcript\tSupport_Count\tSupport_Files\n")
            
            for fingerprint, info in sorted(self.fingerprint_dict.items(), 
                                          key=lambda x: x[1]['count'], reverse=True):
                transcript = info['representative']
                support_files = ",".join(info['support_files'])
                f.write(f"{fingerprint}\t{transcript.transcript_id}\t{info['count']}\t{support_files}\n")
...
def main():
    ...
    if not file_pairs:
        print("未找到匹配的GFF3和FASTA文件对！")
        print("请检查：")
        print("1. 文件扩展名是否正确")
        print("2. 文件名是否匹配（去掉扩展名后相同）")
        print("3. 文件是否存在于指定目录")
        return
    ...
    # 按支持文件数排序
    sorted_fingerprints = sorted(processor.fingerprint_dict.items(), 
                               key=lambda x: x[1]['count'], reverse=True)
    
    print(f"\n支持文件数分布:")
    count_distribution = defaultdict(int)
    for fingerprint, info in sorted_fingerprints:
        count_distribution[info['count']] += 1
    
    for count in sorted(count_distribution.keys(), reverse=True):
        freq = count_distribution[count]
        percentage = (freq / len(processor.fingerprint_dict)) * 100
        print(f"  支持 {count:2d} 个文件: {freq:4d} 个唯一转录本 ({percentage:.1f}%)")
    
    # 显示高支持度的转录本
    high_support_threshold = max(1, len(file_pairs) // 20)  # 支持度前5%
    high_support = [(fp, info) for fp, info in sorted_fingerprints 
                   if info['count'] >= high_support_threshold]
    
    print(f"\n高支持度转录本 (支持文件数 ≥ {high_support_threshold}):")
    for fingerprint, info in high_support[:5]:  # 显示前5个
        transcript = info['representative']
        print(f"  {transcript.transcript_id}:")
        print(f"    染色体: {transcript.chrom}, 链: {transcript.strand}")
        print(f"    外显子数: {len(transcript.exons)}, CDS数: {len(transcript.cds_regions)}")
        print(f"    支持文件数: {info['count']}")
        print(f"    来源文件: {info['source_files'][:3]}{'...' if len(info['source_files']) > 3 else ''}")
