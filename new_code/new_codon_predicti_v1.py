...
TP_ONLY_3 = os.path.join(OUT_DIR, "tp_only_3mer_z.tsv")
TP_ONLY_5 = os.path.join(OUT_DIR, "tp_only_5mer_z.tsv")
SHEET_NAME = "output_candidates"

THREADS = 50
TP_ONLY_KMER_SHUFFLES = 400
TP_ONLY_KMER_MIN_Z = 2.5
TP_ONLY_KMER_ARTIFACT = os.path.join(OUT_DIR, f"tp_only_kmer_shuffle{TP_ONLY_KMER_SHUFFLES}-{TP_ONLY_KMER_MIN_Z}_weights.json")
...
def kozak_logodds(w, pwm, eps=1e-12):
    s = 0.0
    for i, ch in enumerate(w):
        if ch in "ACGT":
            s += math.log2(max(pwm[ch][i], eps))
    return s
def cu_fraction(up):
...
def score_candidates_excel():
    ...
    pwm, weights, std = train_pwm_and_weights(cds_dict, w3, w5)
    ...
    with open(TP_ONLY_KMER_ARTIFACT) as f:
        art = json.load(f)
    df3 = pd.DataFrame(art["3mer"]["weights_z"].items(), columns=["3mer", "z"]).sort_values("z", ascending=False)
    df5 = pd.DataFrame(art["5mer"]["weights_z"].items(), columns=["5mer", "z"]).sort_values("z", ascending=False)
    df3.to_csv(TP_ONLY_3, sep="\t", index=False)
    df5.to_csv(TP_ONLY_5, sep="\t", index=False)                     
