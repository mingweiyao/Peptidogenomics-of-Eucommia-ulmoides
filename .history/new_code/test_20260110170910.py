...
# =========================================================
# Genome background (optional, not used for Kozak now)
# =========================================================
...
# =========================================================
# Motif weights (TP vs TN background)
# =========================================================
...
def compute_motif_weights(lm_tp3, lm_tp5, lm_tn3, lm_tn5, motifs=MOTIFS_DNA, eps=MOTIF_EPS):
    ...
    return weights
# =========================================================
# Training (PWM + LR)  -- [MOD] Use TP-PWM vs TN-PWM for Kozak score
# =========================================================
...
def build_pwm(windows, pseudocount=1.0):
    if not windows: raise ValueError("Kozak windows 为空，无法建 PWM")
    counts = {b: np.full(13, pseudocount, dtype=float) for b in "ACGT"}
    for w in windows:
        if w is None or len(w) != 13: continue
        for i, ch in enumerate(w):
            if ch in counts:                counts[ch][i] += 1.0
    totals = sum(counts[b] for b in "ACGT")
    return {b: (counts[b] / totals).tolist() for b in "ACGT"}
def kozak_logodds_tp_vs_tn(w, pwm_tp, pwm_tn, eps=1e-12):
    s = 0.0
    for i, ch in enumerate(w):
        if ch in "ACGT":
...
def train_pwm_and_lr(df, motif_weights):
    # 1) Build TP/TN Kozak PWM
    tp_ws = []
    tn_ws = []
    for _, r in df.iterrows():
        w = kozak_window(r["seq"], int(r["tis_pos"]))
        if w is None or (not DNA_RE.fullmatch(w)):
            continue
        if int(r["label"]) == 1:
            tp_ws.append(w)
        else:
    ...
    for _, r in df.iterrows():
        ...
        if w is None or up is None: 
            continue
        if not DNA_RE.fullmatch(w): 
            continue
        if not DNA_RE_100.fullmatch(up): 
            continue

        kz = kozak_logodds_tp_vs_tn(w, pwm_tp, pwm_tn)   # [MOD]
        cu = cu_fraction(up)
        ms = motif_score(up, motif_weights)

        if any(np.isnan(v) for v in [kz, cu, ms]): 
            continue
        X.append([kz, cu, ms])
    ...
    weights = {
        ...
        "w1": float(lr.coef_[0][0]),  # kozak_logodds_tp_vs_tn
    ...
    std = {
        "mu": mu.tolist(),
        "sd": sd.tolist(),
        "feat": ["kozak_logodds_tp_vs_tn", "cu_fraction", "motif_score"],
    }
...
def write_lm_tsv(lm_obj, out_tsv):
    ...
    for kmer, logp in lm_obj["logp"].items():
        rows.append({"kmer": kmer, "logp": logp})
    df = pd.DataFrame(rows).sort_values("kmer")
...
def score_candidates_excel(require_5utr=False):
    ...
    for _, r in df_sp.iterrows():
        ...
        # [NOTE] you used kozak_seq in this version
        w = str(r.get("kozak_seq", "")).upper()[:13]
        if len(w) != 13 or (not DNA_RE.fullmatch(w)):
            kozak_scores.append(np.nan); cu_fracs.append(np.nan); motif_scores.append(np.nan)
        ...
        # [MOD] Kozak score now uses TP vs TN PWM
        kz = kozak_logodds_tp_vs_tn(w, pwm_tp, pwm_tn)
        cu = cu_fraction(up)
        ms = motif_score(up, motif_weights)
