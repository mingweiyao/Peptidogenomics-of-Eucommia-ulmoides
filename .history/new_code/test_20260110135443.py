...
def motif_rate(seq, motif):
    ...
    for i in range(denom):
        if seq[i:i+k] == motif:
            cnt += 1
    return cnt / denom if denom > 0 else 0.0 # 为什么要除以denom
def build_motif(df, motifs=MOTIFS_DNA, alpha=0.01):
    out = {}
    for motif in motifs:
        k = len(motif)
        obs = Counter()
        ...
        for _, r in df.iterrows():
            ...
            used_windows += 1
            motif_count = motif_rate(up, motif)
            obs[motif] += motif_count
            total += max(0, len(up) - k + 1)
        V = 4 ** k
        denom = total + alpha * V
        default_p = alpha / denom
        default_logp = math.log(default_p)
        logp = {motif: math.log((c + alpha) / denom) for motif, c in obs.items()}
        out[motif] = {
            "params": {"k": k, "alpha": alpha, "total_windows": int(used_windows), "total_motif_positions": int(total), "V": V },
            "logp": logp,
            "default_logp": float(default_logp),
        }
    return out
def train_tp_tn_lm(df):
    lm_tp = build_motif(df[df["label"]==1], MOTIFS_DNA)
    tp_lm3, tp_lm5 = lm_tp[3], lm_tp[5]
    lm_tn = build_motif(df[df["label"]==0], MOTIFS_DNA)
...
def train_pwm_and_lr(df, genome_bg, lm_tp3, lm_tp5, lm_tn3, lm_tn5, motif_weights):
    ...
    weights = {
        ...
        "w3": float(lr.coef_[0][2]),  # llr3
        "w4": float(lr.coef_[0][3]),  # llr5
        "w5": float(lr.coef_[0][4]),  # motif_score
    }
    std = {
        "mu": mu.tolist(),
        "sd": sd.tolist(),
