...
WORK_BASE_DIR="/data/Eu/bam_for_sp/STAR_bam_sp"
...
run_star_alignment() {
    ...
    if [[ -n "$input2" ]]; then
        ...
        STAR-avx2 --runThreadN $THREADS \
             ...
             --outFileNamePrefix "/data/Eu/bam_for_sp/STAR_bam_sp/${base}_" \
    ...
    samtools view -@ 50 -h -q 255 -b "/data/Eu/bam_for_sp/STAR_bam_sp/${base}_Aligned.sortedByCoord.out.bam" > "/data/Eu/bam_for_sp/STAR_bam_sp/${base}_unique_Aligned.sortedByCoord.out.bam"
    samtools index "/data/Eu/bam_for_sp/STAR_bam_sp/${base}_unique_Aligned.sortedByCoord.out.bam"
    local current_bam="/data/Eu/bam_for_sp/STAR_bam_sp/${base}_unique_Aligned.sortedByCoord.out.bam"
    if [[ -n "$input2" ]]; then
    	featureCounts -T 64 -Q 255 -p --countReadPairs -t exon -g gene_id -a "$GENOME_DIR/$GTF_FILE" -o "/data/Eu/bam_for_sp/STAR_bam_sp/featurecount_sp/${base}_counts.txt" "$current_bam"
    else
    	featureCounts -T 64 -Q 255 -t exon -g gene_id -a "$GENOME_DIR/$GTF_FILE" -o "/data/Eu/bam_for_sp/STAR_bam_sp/featurecount_sp/${base}_counts.txt" "$current_bam"
    fi
}
