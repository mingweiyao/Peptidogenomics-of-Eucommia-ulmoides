...
#                 except Exception as e:
#                     print(f"处理文件 {filename} 时出错: {e}")
# # 6. 提取并合并共有的天然橡胶和绿原酸调控NCP基因对信息                    
# def filter_lines_by_id(data_file_path, id_file_path, output_file_path):
#     with open(id_file_path, 'r') as id_file:
#         target_ids = {line.strip() for line in id_file if line.strip()}
#     matched_lines = []
#     with open(data_file_path, 'r') as data_file:
#         for line in data_file:
#             if line.strip():
#                 parts = line.strip().split('\t')
#                 if len(parts) >= 2 and parts[0] in target_ids:
#                     matched_lines.append(line)
#     with open(output_file_path, 'w') as out_file:
#         out_file.writelines(matched_lines) 
# # 7. 执行所有功能
# def main():
#     folder_path = r"G:\peptidegenomics\test\WGCNA_Results_yield\module_specific_results"
...
#     # 统计天然橡胶合成和绿原酸调控基因连接频率
#     rubber_cga_path = r"G:\peptidegenomics\test\WGCNA_Results_yield\module_rubber_gca"
...
#     # 提取并合并共有的NCP基因对信息
#     id_file = r"G:\peptidegenomics\test\yield\common_id.txt"
#     merge_output = os.path.join(output_base, "common_id_yield_tom.txt")
#     merge_files_by_id(folder_path, id_file, merge_output)
#     # 提取并合并共有的天然橡胶和绿原酸调控NCP基因对信息
#     output_file = os.path.join(output_base, "rubber_gca_tom.txt")
#     filter_lines_by_id(gene_data_output_rubber_cga, id_file, output_file)
#     print("所有任务已完成！")
